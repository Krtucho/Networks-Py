...
#region delegado
# def Brigth(imgpxl, param): # el param es un array [M,L]
#     return param[0]*imgpxl + param[1]
    
    
# def NegGrayImg(imgpxl,param = None):
#     return  255 - imgpxl

# def NegColorImg(imgpxl, param): #el param es la imagen
#     for k in range(0,len(param.shape)):
#          imgpxl[k] = 255 - imgpxl[k]
#     return imgpxl

# def EscInt(imgpxl,param): #el param es un arreglo con [l1, l2, Imax]
#     if imgpxl < param[0] or imgpxl > param[1]:
#         return 0
#     return (imgpxl - param[0])/(param[1]- param[0]) * param[2]

# def FSHS(imgpix,param): #Param es un array [A,B,K]
#     return (param[2]-1)/(param[1]-param[0]) * (imgpix - param[0])


# def ModifPunt(img, f, param): #el param son los parametros de la funcion f
...
# image = ModifPunt(image,FSHS,[m,mi,155])
